...
`docs/smoke_test_log.md`에 Phase별로 기록한다. 4개 컨테이너 전부(0-web,
1-canvas, 2-vision-analysis, 3-pattern-command)를 대상으로 한다.

## 0. 사전 준비

- [ ] `docker compose up --build` 로 4개 컨테이너 기동 (또는 개별 `uvicorn`/`python -m app.main` 실행)
- [ ] 모델 파일 존재 확인: `containers/2-vision-analysis/models/hand_landmarker.task`
...
- [ ] `/health` 4개 컨테이너 전부 `{"status":"ok", ...}` 응답
      (0-web, 2-vision-analysis:8763, 3-pattern-command:8761, 1-canvas:8762)
...
- [ ] 손을 프레임 밖으로 완전히 빼도 어느 컨테이너에서도 크래시/예외 스택이
      로그에 남지 않는다 (특히 2-vision-analysis: `hand_present=False` 처리,
      3-pattern-command: `classifier.reset()` 처리)
...
- [ ] 손을 화면 가장자리(모서리)로 이동 — 랜드마크가 정규화 좌표 `[0,1]`을
      ...
      `containers/1-canvas/app.py:14`)
- [ ] **3-pattern-command 컨테이너를 먼저 내린 상태**(`docker compose stop pattern-command`)에서
      2-vision-analysis가 크래시하지 않고 계속 떠 있는지 확인 (`egress_client.py`의
      지수 백오프 재연결 로그만 남고 프로세스는 생존해야 한다)
- [ ] **1-canvas 컨테이너를 먼저 내린 상태**에서 3-pattern-command가 크래시하지
...
- [ ] 카메라를 한 프레임도 못 읽는 상황(폰 카메라 권한 거부 등)에서 0-web이
...
- [ ] 각 컨테이너를 `Ctrl+C`(로컬 실행 시) 또는 `docker compose down`으로
      ...
      (2-vision-analysis: `SIGINT`/`SIGTERM` 핸들러 등록, Windows에서는
...
- [ ] `pytest containers/2-vision-analysis/tests` 통과
- [ ] `pytest tests/` (레포 루트, 레거시 대상 유닛 테스트) 통과
