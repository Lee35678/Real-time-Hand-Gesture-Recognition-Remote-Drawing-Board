...
우선순위(refactoring.md Pillar 1-1): 환경변수 > config/vision-analysis.{APP_ENV}.yaml
> config/vision-analysis.yaml > 코드 기본값.

`config/` 디렉토리 위치는 실행 환경에 따라 다르다:
- 로컬 개발(리포지토리에서 직접 실행): 리포지토리 루트의 `config/`
- Docker 이미지: `WORKDIR /app` 기준 `/app/config/` (Dockerfile이 `COPY config/vision-analysis*.yaml ./config/`로 배치)

`VISION_CONFIG_DIR` 환경변수로 명시적으로 지정할 수도 있다.
"""
...
def _find_config_dir() -> Path | None:
    override = os.environ.get("VISION_CONFIG_DIR")
    if override:
    ...
    candidates = (
        here.parents[4] / "config" if len(here.parents) > 4 else None,  # repo root (local dev)
        here.parents[2] / "config",  # /app/config (Docker: app/config/loader.py -> /app)
    )
...
def resolve_bool(env_name: str, yaml_path: tuple[str, ...], default: bool) -> bool:
    if env_name in os.environ:
